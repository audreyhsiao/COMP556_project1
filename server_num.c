...
int main(int argc, char **argv) {
  ...
  // The port on which the server should run (on CLEAR, the usable range is
  // 18000 <= port <= 18200).
  ...
  while (1) {
    ...
    // Run through the Linked list of socket
    ...
    // TODO what to do with it when Timeout
    if (select_retval == 0) {
    ...
    {
      if (FD_ISSET(sock, &read_set)) /* check the server socket */ // this is the listening server that is still there
      {
          ...
          if (count <= 0) {
            ...
            if (buf[0] + 1 != count) {
              /* we got only a part of a message, we won't handle this in
                 this simple example */
              printf(
                  "Message incomplete, something is still being transmitted\n");
              return 0;
            } else {
              switch (buf[0]) {
                case 1:
              ...
              //TODO sent pong message: sent same massage back to the client(ip address: current->client_addr.sin_addr)
            }
          }
        }
      }
